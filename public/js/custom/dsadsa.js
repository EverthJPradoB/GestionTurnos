...
ws.onmessage = (event) => {
    ...
    switch (data.action) {
        ...
        case "LISTAR_PANTALLA_GENERAL":
            ...
            // console.log("IDs únicos de venta:", idsVenta);
...
function crearVentanillasGeneral(venta, turnos) {
    ...
    nombreVentanilla = venta.venta_nom;
    ...
    let html = `
        <div class="ventanilla" id="ventanilla_${idVentanilla}">
            <div class="card text-center shadow  sub_ventanilla">
                <h3 class="card-title "> ${nombreVentanilla}</h3> `;
    html += `<div class="">

    <ul class=" mt-3 siguiente tamaño_turno_actual" id="v_${idVentanilla}_siguiente">`;
    j = 1;
    i = 1;
    turnos.forEach(t => {
        if (t.id_venta === idVentanilla && t.id_turn != null) { // Filtramos solo los turnos del idVentaActual

            html += `
            <li class="  turno-pendiente d-flex justify-content-center align-items-center"
            style="gap: 10px; min-height: 40px;">
            <span class="turno-span" id="turno_siguiente_${idVentanilla}_${j}">`;
            if (true) {
...
function crearVentanilla_x_id_venta(turnos, idVenta) {
    ...
    for (let j = 0; j < turnos.length; j++) {
        ...
        v2.append(`
            <div id="turno-v-${turnos[j].id_turn}">
                <span>${turnos[j].turn_code}</span>
            </div>
        `);
    }
...
function removerTurno(turnoId) {
    $(`#turno-v1-${turnoId}`).remove();
    ws.send(JSON.stringify({ action: "LISTA_TURNOS" }));
    // Eliminar el turno de la lista en memoria

    // Vuelve a mostrar los turnos actualizados
    // mostrarTurnos({ turnos_0_4_pantalla: turnos_total_0_4_pantalla });
}
