    ...
    fetch("/usuario/session_venta") // Llama al backend para obtener el id_venta
        .then(response => {
            if (!response.ok) { // Si hay un error
                if (response.status === 401) {
                    throw new Error("No tienes una ventanilla asignada.");
                }
        ...
        .then(data => {
            ...
            socket.onopen = () => {
                ...
                socket.send(JSON.stringify({ action: "LISTA_TURNOS_VENTANILLA", id_venta: id_venta }));

                socket.send(JSON.stringify({ action: "LISTA_SLIDER_TURNOS", id_venta: id_venta }));

                socket.send(JSON.stringify({ action: "TURNOS_PENDIENTES_RESTANTES", id_venta: id_venta }));
            ...
            window.siguienteTurno = function () {
                ...
                socket.send(JSON.stringify({ action: "SIGUIENTE_TURNO", id_venta: id_venta }));
            };


            window.llamarTurno = function () {
                if (turnoActual) {
                    socket.send(JSON.stringify({ action: "LLAMAR_TURNO", turno_id: turnoActual.id_turn, id_venta: id_venta }));
                }
                audio.play().then(() => {
            ...
            window.finalizarTurno = function () {
                if (turnoActual && turnoActual.id_turn) {
                    ...
                    socket.send(JSON.stringify({ action: "FINALIZAR_TURNO", turno_id: turnoActual.id_turn, id_venta: id_venta, id_per: data.id_per }));
                    //  turnoActual = null;
                ...
                if (turnoActual && turnoActual.id_turn) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            finalizarTurno_temporizador();
                            socket.send(JSON.stringify({ action: "CANCELAR_TURNO", turno_id: turnoActual.id_turn, id_venta: id_venta, id_per: data.id_per }));
                            //  turnoActual = null;
                            $("#turno_codigo, #turno_ventanilla, #turno_tramite, #turno_espera, #turno_estado, #turno_actual").text("-");
                            $("#btnLlamarTurno, #btnFinalizarTurno,#btnAnularTurno,#btnCancelarTurno").prop("disabled", true);
                ...
                if (turnoActual && turnoActual.id_turn) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            finalizarTurno_temporizador();
                            socket.send(JSON.stringify({ action: "AUSENTE_TURNO", turno_id: turnoActual.id_turn, id_venta: id_venta, id_per: data.id_per }));
                            //  turnoActual = null;
                            $("#turno_codigo, #turno_ventanilla, #turno_tramite, #turno_espera, #turno_estado, #turno_actual").text("-");
                            $("#btnLlamarTurno, #btnFinalizarTurno,#btnAnularTurno,#btnCancelarTurno").prop("disabled", true);
                            ...
                            Swal.fire({
                                ...
                                text: "El turno ha sido marcado como ausente correctamente.",
                                icon: "success"
                            });
                        }
                    });
            ...
            socket.onmessage = (event) => {
                ...
                if (data.action === "LISTA_SLIDER_TURNOS") {
                    ...
                    if (turnos_slider) {
                       ...
                       <button class="btn btn-sm priorizar-btn" 
            style="background-color: #6F42C1; color: white; border: none;" 
            data-id="${t.id_turn}" 
            onclick="priorizarTurno(${t.id_turn})">Priorizar</button>
            ...
            document.getElementById('pri_form').addEventListener('submit', (e) => {
                ...
                socket.send(JSON.stringify({ action: "PRIORIZAR_TURNO", formData }));
        ...
        .catch(error => {
            ...
            mensajeError.style.display = "block"; // Hace visible el mensaje
        });
